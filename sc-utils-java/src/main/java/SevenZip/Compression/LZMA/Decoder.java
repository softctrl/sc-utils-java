...
public class Decoder implements IProcessor<DecodeParameter> {
	 ...
	 * @throws IOException
	 */
	private Decoder readProperties(InputStream inputStream) throws IOException {
		int propertiesSize = 5;
		byte[] properties = new byte[propertiesSize];
		if (inputStream.read(properties, 0, propertiesSize) != propertiesSize){
			throw new RuntimeException("input .lzma stream is too short");
		}
	 ...
	 */
	@Override
	public boolean Code(DecodeParameter parameter) throws IOException {
		return this.Code(parameter.inStream, parameter.outStream/*, parameter.outSize*/);
	}
