...
public class Encoder implements IProcessor<EncodeParameter> {
	...
	int ReadMatchDistances() throws java.io.IOException {
		...
		if (_numDistancePairs > 0) {
			...
			if (lenRes == _numFastBytes)
				lenRes += _matchFinder.GetMatchLen((int) lenRes - 1, _matchDistances[_numDistancePairs - 1], Base.kMatchMaxLen - lenRes);
		}
		_additionalOffset++;
		return lenRes;
	}
	...
	int GetRepLen1Price(int state, int posState) {
		return SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]) + SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetOptimum(int position) throws IOException {
		...
		_optimum[1].Price = SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) + _literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!Base.StateIsCharState(_state), matchByte, currentByte);
		...
		while (true) {
			...
			int curAnd1Price = curPrice + SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) + _literalEncoder.GetSubCoder(position, _matchFinder.GetIndexByte(0 - 2)).GetPrice(!Base.StateIsCharState(state), matchByte, currentByte);
			...
			if (!nextIsChar && matchByte != currentByte) {
				...
				if (lenTest2 >= 2) {
					...
					int nextRepMatchPrice = curAnd1Price + SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) + SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
					{
			...
			for (int repIndex = 0 ; repIndex < Base.kNumRepDistances ; repIndex++) {
				...
				if (lenTest < numAvailableBytesFull) {
					...
					if (lenTest2 >= 2) {
						...
						int curAndLenCharPrice = repMatchPrice + GetRepPrice(repIndex, lenTest, state, posState) + SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) + _literalEncoder.GetSubCoder(position + lenTest, _matchFinder.GetIndexByte(lenTest - 1 - 1)).GetPrice(true, _matchFinder.GetIndexByte(lenTest - 1 - (reps[repIndex] + 1)), _matchFinder.GetIndexByte(lenTest - 1));
					...
					if (lenTest == _matchDistances[offs]) {
						if (lenTest < numAvailableBytesFull) {
							...
							if (lenTest2 >= 2) {
								...
								int posStateNext = (position + lenTest) & _posStateMask;
								int curAndLenCharPrice = curAndLenPrice + SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) + _literalEncoder.GetSubCoder(position + lenTest, _matchFinder.GetIndexByte(lenTest - 1 - 1)).GetPrice(true, _matchFinder.GetIndexByte(lenTest - (curBack + 1) - 1), _matchFinder.GetIndexByte(lenTest - 1));
								state2 = Base.StateUpdateChar(state2);
								posStateNext = (position + lenTest + 1) & _posStateMask;
								int nextMatchPrice = curAndLenCharPrice + SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
								int nextRepMatchPrice = nextMatchPrice + SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
	...
	/*
	 * (non-Javadoc)
	 * @see SevenZip.Compression.LZMA.IProcessor#Code(SevenZip.Compression.LZMA.IParameter)
	 */
	@Override
	public boolean Code(EncodeParameter parameter) throws IOException {
		return this.Code(parameter.inStream, parameter.outStream, parameter.inSize, parameter.outSize, parameter.progress);
	}

	boolean _outputStremPrepared = false;

	/*
	 * (non-Javadoc)
	 * @see SevenZip.Compression.LZMA.IProcessor#prepareOutputStream(java.io.OutputStream, long)
	 */
	public Encoder prepareOutputStream(java.io.OutputStream outStream, long fileSize) throws IOException {

		this.WriteCoderProperties(outStream);
		if (this._writeEndMark)
			fileSize = -1;
		for (int i = 0 ; i < 8 ; i++)
			outStream.write((int) (fileSize >>> (8 * i)) & 0xFF);
		_outputStremPrepared = true;
		return this;
	 ...
	 * @throws IOException
	 */
	public boolean Code(java.io.InputStream inStream, java.io.OutputStream outStream, long inSize, long outSize, ICodeProgress progress) throws IOException {

		boolean result = false;
		if (!_outputStremPrepared)
			throw new RuntimeException("You need to prepare the output stream first.");
		_needReleaseMFStream = false;
		try {
